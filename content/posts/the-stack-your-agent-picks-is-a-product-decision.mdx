...
I see this with The Keep. It is a markdown-native multi-agent system because that is the shape I chose: agents, rubrics, source-of-truth files, and workflow state live in markdown or YAML. TypeScript shows up where I/O makes it pay rent: Gmail, Discord, RSS, captions, GitHub Actions. If an agent working in that repo suggests a database-backed dashboard for every new workflow, it is missing the local rule. If it writes another markdown skill file that fits the system, it has learned the room.
...
At TeachMe.To, I owned a product area where the technical choice and the product choice were the same conversation. I was not just shipping React and APIs. I was looking at retention cohorts, finding a 16 percentage-point gap, and turning that into a product pivot. I found a 40 percent conversion drop-off in a funnel and had to think about what work would move the business, not what work would feel clean in a pull request.

That changes how I hear "just build the feature flags."

If flags decide which cohort sees which marketplace flow, they are part of the analytics story. If permissions decide who can approve a venue, warn a partner, or change location data, they are part of operations. If a rate limit hits a paid instructor during a booking spike, the implementation detail has become a business problem.

Same with Stripe. I have built subscription and webhook flows. I have also built payment event processing as a separate Go service. Payment code punishes cute architecture. You care about retries, event ordering, idempotency, reconciliation, and boring logs. An agent recommending custom ownership for a payment-adjacent surface should trigger the same response as a junior engineer saying it in planning: maybe, but show me the failure path first.
